...
// gives, and the one attempt picks among them.
//
...
func Choose(ctx context.Context, resolve func(context.Context) (Candidates, error), attempt, bound int, now time.Time, wait time.Duration) (ref Ref, until time.Time, err error) {
	...
	ref, err = candidates.Attempt(attempt, bound)
