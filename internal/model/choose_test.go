...
func TestChoose(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"no stays is the first candidate", two, 0, first, time.Time{}, nil},
		{"a stay is the next candidate", two, 1, second, time.Time{}, nil},
		...
		t.Run(tc.name, func(t *testing.T) {
			ref, until, err := model.Choose(context.Background(), tc.resolve, tc.stays, 2, now, wait)
