...
**12. Pay-as-you-go spend is capped by the balance, not by code.** The overflow budget is bounded by keeping the account's balance at the intended ceiling with automatic recharge disabled. Vendor-enforced, impossible to undercount, and it accounts for interactive use, which an in-agent cap could not. The consequence accepted knowingly: exhaustion is indistinguishable from any other transient failure, and is handled as one.
