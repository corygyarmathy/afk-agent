...
| `implement-watch` | `watching` | Reads CI's check runs on the pushed head, and the checks the base branch's rulesets require. Unfinished, or a required check with no run yet: looks again after `--ci-wait`, until `--ci-ceiling`, then hand-back. Green, every run passed and every required check among them: on to the review. Red: back to the session, with what CI said, until `--ci-rounds` runs out, then hand-back. |
